import { Component, Renderer2, ElementRef, OnInit, ChangeDetectorRef, Input, OnChanges, SimpleChanges } from '@angular/core';
...
export class TimePomoComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['focusDuration'] || changes['shortBreakDuration'] || changes['longBreakDuration']) {
      if (!this.isRunning) {
  ...
  updateProgress(): void {
    const progressPercentage = this.totalSeconds > 0 ? ((this.minutes * 60) + this.seconds) / this.totalSeconds : 1;
